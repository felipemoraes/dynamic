...
public class BM25 implements WeightingModel {	
	...
	public HashMap<String, HashMap<String, Double>> getDetails(Explanation explanation) {
		HashMap<String, HashMap<String, Double>> details = new HashMap<String, HashMap<String, Double>>();
		HashMap<String, List<Double>> lists = new HashMap<String, List<Double>>();
		List<String> terms = new ArrayList<String>();
		details.put("IDF", new HashMap<String, Double>());
		details.put("TF", new HashMap<String, Double>());
		details.put("DL", new HashMap<String, Double>());
		details.put("ADL", new HashMap<String, Double>());
		...
		for (int i = 0; i < terms.size(); i++) {
			details.get("IDF").put(terms.get(i),lists.get("IDF").get(i));
			details.get("TF").put(terms.get(i),lists.get("TF").get(i));
			details.get("DL").put(terms.get(i),lists.get("DL").get(i));
			details.get("ADL").put(terms.get(i),lists.get("ADL").get(i));
		}
